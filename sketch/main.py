...
def parse():
    ...
    parser.add_argument('-mn', '--min_length', type=int, default=5, help='min length of sequence')
...
def run(args):
    ...
    if args.train and not args.load:
        ...
        n_layers, hidden_size = parseFilename(args.sketch_model)
        runTest(args.test, n_layers, hidden_size, embed_size, attr_size, attr_num, overall, 
            args.sketch_model, args.topic_model, beam_size, max_length, min_length, save_dir)
