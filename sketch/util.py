...
def batch2TrainData(vocab, pair_batch, evaluation=False):
    ...
    for i in range(len(pair_batch)):
        ...
        # exit(0)
        # print(pair_batch[i][1])
        # if vocab.topic2idx.get(pair_batch[i][1]) is None:
        #     print(pair_batch[i])
        #     exit(0)

        topic_batch.append(pair_batch[i][1])
        # topic_batch.append(vocab.topic2idx[pair_batch[i][1]])
