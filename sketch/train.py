...
def train(attr_input, topic_input, sketch_input, sketch_output, mask, encoder, sketch_decoder, encoder_optimizer, sketch_decoder_optimizer):
    ...
    # print(sketch_decoder_output)
    # print(sketch_decoder_output.shape)
...
def evaluate(attr_input, topic_input, sketch_input, sketch_output, mask, encoder, sketch_decoder, encoder_optimizer, sketch_decoder_optimizer, save_output=False):
    ...
    # print("blue-1 in one batch:{}".format()
    # print("blue-1 in one batch:{}".format(cal_blue(sketch_decoder_output.transpose(0,1), sketch_output.transpose(0,1),mask)))
    if not save_output:
        return sum(print_losses), cal_blue(torch.max(sketch_decoder_output,-1).indices.transpose(0,1), sketch_output.transpose(0,1),mask)
    else:
        return sum(print_losses), cal_blue(torch.max(sketch_decoder_output,-1).indices.transpose(0,1), sketch_output.transpose(0,1),mask), torch.max(sketch_decoder_output,-1).indices.transpose(0,1)
...
def trainIters(corpus, learning_rate, lr_decay_epoch, lr_decay_ratio, batch_size, n_layers, hidden_size, 
    ...
    while True:
        ...
        for val_batch in val_batches:
            ...
            loss, blue = evaluate(attr_input, topic_input, sketch_input, sketch_output, mask, encoder, sketch_decoder, encoder_optimizer, sketch_decoder_optimizer)
            vl_blue += blue
            vl_loss += loss
        vl_loss /= len(val_batches)
        vl_blue /= len(val_batches)
        
        writer.add_scalar("Valid/loss", vl_loss, step)

        print('\n' + '-' * 30)
        print('valid | epoch {:3d} | valid loss {:5.5f} | valid ppl {:8.3f} | vaild blue-1 {}'.format(epoch, vl_loss, math.exp(vl_loss), vl_blue))
        print('-' * 30)
        
        # Save the model if the validation loss is the best we've seen so far.
        model_path = os.path.join('G:/sketch', "model")
        if not best_val_loss or vl_loss < best_val_loss:
            ...
            for test_batch in test_batches:
                ...
                loss, blue, predict = evaluate(attr_input, topic_input, sketch_input, sketch_output, mask, encoder, sketch_decoder, encoder_optimizer, sketch_decoder_optimizer, save_output=True)
            ...
            ts_blue /= len(test_batches)

            writer.add_scalar("Test/loss", ts_loss, step)
            
            print('\n' + '-' * 30)
            print('| test loss {:5.2f} | test ppl {:8.2f} | test blue1 {}'.format(ts_loss, math.exp(ts_loss),ts_blue))
            print('-' * 30 + '\n')
        ...
        if vl_loss > best_val_loss:
            ...
            print('Save predict as {}'.format('sketch_predict.pkl'))
            pickle.dump(predicts, open('sketch_predict.pkl','wb'))
