...
def beam_decode(birnn_encoder, review_decoder, topics, sketchs, review_decoder_hidden, review_encoder_out, vocab):
    ...
    nti = vocab.topic2idx['<eos>']
    topic = Variable(torch.LongTensor([[nti]]))
    topic = topic.cuda() if USE_CUDA else topic

    sketch_tokens = [vocab.sketch2idx[t] for t in sketchs[0]]
    ...
    for i in range(seq_len):
        ...
        review_output, review_hidden, _ = review_decoder(review_input, review_hidden, rnn_output, topic, sketch,
                                                         review_encoder_out)
        if vocab.idx2sketch[int(sketch_out[i][0])] in pos:
            ...
            review_input = review_input.cuda() if USE_CUDA else review_input
        elif vocab.idx2sketch[int(sketch_out[i][0])] != EOS_ID:
            review.append(vocab.idx2sketch[int(sketch_out[i][0])])
