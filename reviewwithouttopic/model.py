...
class Attn(nn.Module):
    ...
    def forward(self, hidden, encoder_outputs):
        ...
        attn_energies = torch.tanh(self.score(H, encoder_outputs)) # compute attention score [B,N,T]
...
class TopicAttnDecoderRNN(nn.Module):
    ...
    def forward(self, input_seq, last_hidden, encoder_out):
        ...
        topic_embedded = self.embedding_dropout(topic_embedded)

        # Get current hidden state from input word and last hidden state
        rnn_output, hidden = self.gru(topic_embedded,
                                      last_hidden)  # rnn_output: [N=seq_len, B, H]  hidden: [n_layer, B, H]
...
class SketchAttnDecoderRNN(nn.Module):
    def __init__(self, topic_embedding, sketch_embedding, embed_size, hidden_size, attr_size, output_size, n_layers=1,
                 dropout=0.2):
