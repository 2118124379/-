...
#
#         # for electronic
#         while (i < 4 and int(topi[0][i]) < 3 and len(decoded_topics) < MIN_TOPIC) or (i < 4 and vocab.idx2topic[int(topi[0][i])] in decoded_topics):
#             i += 1
#
#         nti = int(topi[0][i])
#         if nti == EOS_ID:
...
#             sketch_tokens = [vocab.idx2sketch[int(t)] for t in sketch_tokens]
...
def decode(topic_decoder, sketch_decoder, topic_decoder_hidden, sketch_decoder_hidden, topic_encoder_out,
           sketch_encoder_out, vocab, max_length, min_length):
    topic_input = Variable(torch.LongTensor([[SOS_ID]]))
    topic_input = topic_input.cuda() if USE_CUDA else topic_input
...
def beam_decode(sketch_decoder, sketch_decoder_hidden,
                sketch_encoder_out, topics, vocab, beam_size, max_length, min_length):
    sketch_hidden = sketch_decoder_hidden

    decoded_sketchs = []


    topic = Variable(torch.LongTensor([[2]]))
    topic = topic.cuda() if USE_CUDA else topic
    # print(topic)
    hyps = [Hypothesis(tokens=[SOS_ID], log_probs=[0.0], state=sketch_hidden) for _ in range(beam_size)]
    results = []
    ...
    while steps < max_length and len(results) < beam_size:
        ...
        for hyp in hyps:
            ...
            sketch_hidden = hyp.state
            # print(sketch_hidden)
            sketch_output, sketch_hidden, _ = sketch_decoder(sketch_input, sketch_hidden, topic,
                                                             sketch_encoder_out)
            new_hiddens.append(sketch_hidden)
    ...
    return decoded_sketchs


def evaluate(sketch_encoder, sketch_decoder, vocab, pair, beam_size, max_length,
             min_length):
    ...
    if beam_size == 1:
        return decode(sketch_decoder, sketch_decoder_hidden,
                      sketch_encoder_out, pair[4], vocab, max_length, min_length)
    else:
        return beam_decode(sketch_decoder, sketch_decoder_hidden, sketch_encoder_out, pair[4], vocab, beam_size,
                           max_length, min_length)


def evaluateRandomly(topic_encoder, topic_decoder, sketch_encoder, sketch_decoder, vocab, user_rdict, item_rdict, pairs,
                     n_pairs, beam_size, max_length, min_length, save_dir):
    path = os.path.join(save_dir, 'decode')
    if not os.path.exists(path):
        os.makedirs(path)
    f1 = open(path + "/newdecoded.txt", 'w')
    new_data = []
    for i in range(n_pairs):
        ...
        if beam_size >= 1:
            ...
            output_sketchs = evaluate(sketch_encoder, sketch_decoder, vocab, pair,
                                      beam_size, max_length, min_length)

            topic_sentence = ' '.join(pair[4])
            sketch_words = []
            ...
            new_data.append({
                'reviewerID': user_rdict[user],
                'asin': item_rdict[item],
                'overall': rating + 1,
                'topic_tok': pair[1],
                'sketchText': sketchs,
                'topic_gen': pair[4],
                'sketch_gen': sketch_sentence,
                'text': '||'.join([' '.join(s[1:-1]) for s in pair[3]])
            })
    f1.close()
    json.dump(new_data, open('./test_tok.json', 'w'))


def runTest(corpus, n_layers, hidden_size, embed_size, attr_size, attr_num, overall, sk_modelFile, tp_modelFile,
            beam_size, max_length, min_length, save_dir):
    ...
    evaluateRandomly(topic_encoder, topic_decoder, sketch_encoder, sketch_decoder, vocab, user_rdict, item_rdict,
                     test_pairs, len(test_pairs), beam_size, max_length, min_length, save_dir)
