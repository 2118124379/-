...
def cal_blue(output,gr,mask):
    ol = []
    mgr = []
    for batch_index in range(output.shape[0]):
        model_output = output[batch_index].tolist()
        grn = gr[batch_index].tolist()
        pad = grn[-1]
        while len(model_output) and model_output[-1] == pad:
            model_output.pop()
        while len(grn) and grn[-1] == pad:
            grn.pop()

        ol.append(model_output)
        mgr.append([grn])
    # print(mgr)
    # print(ol)
    blue_score = compute_bleu(mgr, ol, 1)[0]
    return blue_score


def train(topic_input, sketch_output, review_input, review_output, review_decoder, review_decoder_optimizer,mask):

    # encoder_optimizer.zero_grad()
    # birnn_encoder_optimizer.zero_grad()
    review_decoder_optimizer.zero_grad()    # 梯度置零
    topic_size = torch.tensor([len(i) for i in topic_input])
    sketch_size = torch.tensor([len(i) for i in sketch_output])
    ...
    review_decoder_output, _1, _2, _3, _4, _5 = review_decoder(topic_input, topic_size,sketch_output, sketch_size,review_input, review_size)
    # print(review_decoder_output)
    # print(review_decoder_output.shape)
    # print(review_output)
    # print(review_output.shape)
    # print(mask)
    # exit(0)
    # print(review_decoder_output, review_output)
    # print(sum(review_decoder_output))
    ...
    clip = 5.0
    # ec = torch.nn.utils.clip_grad_norm_(filter(lambda p: p.requires_grad, encoder.parameters()), clip)
    # bc = torch.nn.utils.clip_grad_norm_(filter(lambda p: p.requires_grad, birnn_encoder.parameters()), clip)
    # dc = torch.nn.utils.clip_grad_norm_(filter(lambda p: p.requires_grad, review_decoder.parameters()), clip)

    # encoder_optimizer.step()
    # birnn_encoder_optimizer.step()
    review_decoder_optimizer.step()
    # print(len(print_losses))
    return sum(print_losses) / len(print_losses)

def evaluate(topic_input, sketch_output, review_input, review_output, review_decoder, review_decoder_optimizer,mask, output_blue=False):
    ...
    review_decoder_output, _1, _2, _3, _4, _5 = review_decoder(topic_input, topic_size, sketch_output, sketch_size,
                                                               review_input, review_size)
    mask_loss = masked_cross_entropy(review_decoder_output, review_output, mask)
    loss += mask_loss
    print_losses.append(mask_loss.item())

    if output_blue:
        return sum(print_losses) / len(print_losses), cal_blue(torch.max(review_decoder_output, -1).indices.transpose(0, 1), review_input.transpose(0, 1), mask)
    else:
        return sum(print_losses) / len(print_losses)
...
def trainIters(corpus, learning_rate, lr_decay_epoch, lr_decay_ratio, batch_size, n_layers, hidden_size, 
    ...
    # topic encoder
    # save_dir = '/home/czf/Code/Coarse-to-Fine-Review-Generation'
    ...
    # aspect_ids = nn.Embedding(vocab.n_topics, 100)  # remove [SOS] [EOS] [PAD]
    # aspect_ids.weight.data.copy_(torch.from_numpy(np.array(ids)))
    # aspect_ids.weight.data = torch.tensor(ids,dtype=torch.int32)
    # aspect_ids.weight.requires_grad = False
    
    # attn_model = 'dot'
    review_decoder = mymodel.Transformer(vocab.n_topics,100,vocab.n_sketchs,100,vocab.n_words,100)
    ...
    while True:
        ...
        for batch_idx, training_batch in enumerate(training_batches):
            ...
            for i in range(review_input.size(0)):
                for j in range(review_input.size(1)):
                    if int(review_input[i][j]) == PAD_ID and int(review_output[i][j]) != PAD_ID:
                        print("1111111111111111111111111111111111111111111111111111111111")
                        exit(0)
            if review_input.size(1)>100:
                los_num += 1
                continue
            print(topic_input.size(),sketch_output.size(),review_input.size(),review_output.size())
            loss = train(topic_input, sketch_output, review_input, review_output,review_decoder,
                            review_decoder_optimizer,mask)
            step += 1
            ...
            print("epoch {} batch {} loss={} perplexity={} de_lr={:05.5f}".format(epoch, batch_idx,
            loss, math.exp(loss),
            review_decoder_optimizer.param_groups[0]['lr']))

        cur_loss = tr_loss / (len(training_batches)-los_num)
        print(cur_loss)
        print('\n' + '-' * 30)
        ...
        for val_batch in val_batches:
            ...
            if review_input.size(1)>100:
                los_num += 1
                continue
            loss = evaluate(topic_input, sketch_output, review_input, review_output,review_decoder,
                            review_decoder_optimizer,mask)
            
            vl_loss += loss
        vl_loss /= (len(val_batches)-los_num)
        ...
        if not best_val_loss or vl_loss < best_val_loss:
            ...
            ts_loss = 0
            tr_blue = 0
            # There insert data accidiently
            # real_sketch_output = pickle.load(open('sketch_predict.pkl','rb'))
            # test_batch_index = 0
            los_num = 0
            for test_batch in test_batches:
                attr_input, topic_input, sketch_output, review_input, review_output, mask = test_batch
                # attr_input, topic_input, _, review_input, review_output, mask = test_batch
                # sketch_output = real_sketch_output[test_batch_index].transpose(0, 1)
                # test_batch_index += 1
                if review_input.size(1) > 100:
                    los_num += 1
                    continue
                loss,blue_score = evaluate(topic_input, sketch_output, review_input, review_output,review_decoder,
                            review_decoder_optimizer,mask,output_blue=True)
                
                ts_loss += loss
                tr_blue += blue_score
            ts_loss /= (len(test_batches)-los_num)
            tr_blue /= (len(test_batches)-los_num)

            writer.add_scalar("Test/loss", ts_loss, step)
            writer.add_scalar('Test/blue',tr_blue, step)
            
            print('\n' + '-' * 30)
            print('| test loss {:5.2f} | test ppl {:8.2f} | test blue {: 8.2f}'.format(ts_loss, math.exp(ts_loss),
                                                                                       tr_blue*100))
            print('-' * 30 + '\n')
